...
(triangles) error in depth at $t = T/2$ versus cell size $\Delta x$,
log-log, with order-1 and order-2 reference slopes (dashed); the
overall fitted slopes are 1.73 (L1) and 1.58 (L2). (b) Order observed
between successive meshes, $\log_2(e_{\text{coarse}}/e_{\text{fine}})$,
plotted against the finer mesh of each pair. Panel (b) carries the
claim panel (a) cannot: a single fitted slope averages the degradation
away, whereas the pairwise orders show it directly, falling from 1.81
to 1.59 in L1 and from 1.71 to 1.43 in L2 as the moving wet/dry
shoreline — locally first order — comes to dominate the shrinking
smooth-region error. Generated by `fig05_convergence.R` from the
`gen_convergence` example.
