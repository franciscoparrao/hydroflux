"""Plot the hydroflux-solver-1d demo on the Río Huasco reach.

Mirrors `maule_reach_demo/plot_results.py`. Only the title and the
location of the OUT_DIR differ — the visualisation is intentionally
identical so the two demos can be compared side by side without
chart-style confounds.
"""
...
def main() -> None:
    ...
    ax_top.fill_between(x_km, bed, water_surface, color="#dcc69b", alpha=0.85,
                        label="Water depth")
    ax_top.plot(x_km, water_surface, color="#8b6a1a", lw=1.4,
                label="Water surface")
    ...
    ax_top.set_title(
        "Río Huasco reach — hydroflux-solver-1d steady state "
        f"(n = {n}, dx = {dx:.1f} m, q = 1 m²/s, n = 0.06)",
        fontsize=10, loc="left",
    )

    ax_bot.axhline(1.0, color="#a04040", lw=0.8, ls="--",
                   label="Critical (Fr = 1)")
    ax_bot.plot(x_km, froude, color="#8b6a1a", lw=1.2)
