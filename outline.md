...
Estado: Año 1, solver-2d orden 2 + UK EA suite 6/6 + **Track A scaffolding cerrado (milestone 2027 Q4 ADELANTADO)** + lake-at-rest sobre bumpy bed + lake-at-rest sobre smooth Thacker bed (Castro & Parés 2007 deuda DESCARGADA) + **Discharge-BC-on-dry fix vía Manning normal depth ghost + bcs-aware CFL** (deuda UK EA suite DESCARGADA). ~221 tests verde. Solver-2d features: MUSCL + SSP-RK2 + Liang & Marche 2009 bed-recon + flux-rescaling + Manning + point source + rain-on-grid + Manning-normal-depth ghost para Discharge-on-dry. Autograd features: forward-mode `Dual` + `Real` trait + primitivas SWE genéricas + 1D Lax-Friedrichs sobre `T: Real` + demo de calibración Manning (4 iter a precisión de máquina). 4 analytical benchmarks + 6 UK EA-style + AD-vs-FD locking test + 2 well-balanced lake-at-rest tests + cold-start Discharge-on-dry test.
Próximo milestone: Track C (GPU port via wgpu, outline 2027 Q3) o Track A application (calibración sobre data Santa Juana real).
