...
Estado: Año 1, solver-2d orden 2 + UK EA suite 6/6 + Track A scaffolding cerrado + lake-at-rest bumpy + Thacker (Castro DESCARGADA) + Discharge-on-dry (UK EA deuda DESCARGADA) + **Track A application iter 1 (synth bed) + iter 2 (DEM bed) + iter 3 (DEM + tiempo real)**: Manning calibration sobre evento Atacama 2017, iter 3 convergencia en 9 iter a |err|=3.14e-8 sobre 600k pasos CFL reales por forward. ~221 tests verde + 4 demos AD ejecutables. Solver-2d features: MUSCL + SSP-RK2 + LM-2009 bed-recon + flux-rescaling + Manning + point source + rain-on-grid + h_n ghost. Autograd features: `Dual` + `Real` trait + primitivas SWE genéricas + 1D Lax-Friedrichs sobre `T: Real` + 4 demos calibración (synth precisión máquina + Atacama 2017 synth-bed + DEM-bed + DEM-bed-realtime).
Próximo milestone: cierre Track A application — validation split aguas arriba + rating curve DGA real (target observado vs twin) + channel width DEM-derived.
...
| **2027 Q4** ✅ (scaffolding + application iter 1-3) | **TRACK A (Autograd)**: Forward-mode dual numbers ✅. **Application iter 1 ✅** synth bed, **iter 2 ✅** DEM bed, **iter 3 ✅** DEM + tiempo real no comprimido (21 días reales, 600k pasos CFL/forward, convergencia en 9 iter a \|err\|=3.14e-8). **Adelantado a 2026-05-23**. Falta para cierre completo: validation aguas arriba + rating curve real + channel width DEM-derived. Decisión de ángulo del paper 2028 Q1 sigue diferida a 2027 Q4 real. |
