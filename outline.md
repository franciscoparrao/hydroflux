...
Estado: Año 1, solver-2d orden 2 + UK EA suite 6/6 + Track A scaffolding cerrado + lake-at-rest bumpy + Thacker (Castro DESCARGADA) + Discharge-on-dry (UK EA deuda DESCARGADA) + **Track A application iter 1-5 cerradas**: synth bed → DEM bed → tiempo real → rating curve target → width DEM-derived. Iter 3 (twin): |err|=3.14e-8. Iter 5 (rating + width DEM): n_recovered=0.0244 al borde envelope Chow, shape mismatch atribuído a wide-channel 1D approximation (compound cross-section pendiente). ~221 tests verde + 6 demos AD ejecutables.
Próximo milestone: cierre Track A application — validation split aguas arriba + rating curve oficial DGA + compound cross-section solver extension.
...
| **2027 Q4** ✅ (scaffolding + application iter 1-5) | **TRACK A (Autograd)**: Forward-mode dual numbers ✅. **Application iter 1-5 ✅**: synth bed → DEM bed → tiempo real → rating curve target → width DEM-derived. iter 5 confirma que width es driver parcial (n 0.017→0.024 al borde envelope) pero NO arregla shape mismatch (RMSE unchanged). **Adelantado a 2026-05-23**. Falta para cierre completo: validation aguas arriba + rating curve oficial DGA + compound cross-section en el solver (nuevo pendiente, bloqueado por solver extension). |
