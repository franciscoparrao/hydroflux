...
Estado: Año 1, solver-2d orden 2 + UK EA suite 6/6 + **Track A scaffolding cerrado (milestone 2027 Q4 ADELANTADO)** + lake-at-rest sobre bumpy bed + lake-at-rest sobre smooth Thacker bed (Castro & Parés 2007 deuda DESCARGADA, no era necesario). ~217 tests verde. Solver-2d features: MUSCL + SSP-RK2 + Liang & Marche 2009 bed-recon + flux-rescaling + Manning + point source + rain-on-grid. Autograd features: forward-mode `Dual` + `Real` trait + primitivas SWE genéricas + 1D Lax-Friedrichs sobre `T: Real` + demo de calibración Manning (4 iter a precisión de máquina). 4 analytical benchmarks + 6 UK EA-style + AD-vs-FD locking test + 2 well-balanced lake-at-rest tests (bumpy + smooth).
Próximo milestone: Discharge-BC-on-dry fix via Manning normal depth ghost (deuda pendiente de UK EA suite), después Track C (GPU port via wgpu, outline 2027 Q3) o Track A application (calibración sobre data Santa Juana real).
...
| **2027 Q4** ✅ (scaffolding) | **TRACK A (Autograd)**: Forward-mode dual numbers ✅ (crate `autograd` con `Dual`, `Real`, primitivas SWE genéricas, solver SWE 1D LF, demo calibración Manning). Falta para cierre completo: aplicar a cuenca chilena real (Santa Juana / Atacama 2017). **Adelantado a 2026-05-23**. Decisión de ángulo del paper 2028 Q1 (ver § Estrategia) sigue diferida a 2027 Q4 real. |
