...
Estado: Año 1, solver-2d orden 2 + UK EA suite 6/6 + Track A scaffolding cerrado + lake-at-rest bumpy + lake-at-rest Thacker (Castro & Parés DESCARGADA) + Discharge-on-dry fix (UK EA deuda DESCARGADA) + **Track A application iter 1: Manning calibration sobre evento Atacama 2017 con forzamiento REAL del DGA**. ~221 tests verde + 2 demos AD ejecutables. Solver-2d features: MUSCL + SSP-RK2 + LM-2009 bed-recon + flux-rescaling + Manning + point source + rain-on-grid + h_n ghost. Autograd features: `Dual` + `Real` trait + primitivas SWE genéricas + 1D Lax-Friedrichs sobre `T: Real` + Manning calibration demos (sintético precisión máquina + Atacama 2017 con data DGA real, recuperando n=0.04 a |err|=1.6e-5).
Próximo milestone: cierre Track A application (DEM cross-section + comparación contra rating curve DGA observada + validation split aguas arriba) o Track C alternativo (software rasterizer / cloud burst).
...
| **2027 Q4** ✅ (scaffolding + application iter 1) | **TRACK A (Autograd)**: Forward-mode dual numbers ✅ (crate `autograd` con `Dual`, `Real`, primitivas SWE genéricas, solver SWE 1D LF, demo calibración Manning). **Application iter 1 ✅**: Manning calibration sobre evento Atacama 2017 con forzamiento DGA real, recuperando n=0.04 a 0.04% rel error. **Adelantado a 2026-05-23**. Falta para cierre completo del Track A: DEM cross-section + tiempo real + validation aguas arriba + rating curve real. Decisión de ángulo del paper 2028 Q1 (ver § Estrategia) sigue diferida a 2027 Q4 real. |
