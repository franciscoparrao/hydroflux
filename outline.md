...
Estado: Año 1, solver-2d orden 2 + UK EA suite 6/6 + Track A scaffolding cerrado + lake-at-rest bumpy + Thacker (Castro DESCARGADA) + Discharge-on-dry (UK EA deuda DESCARGADA) + **Track A application iter 1-8 COMPLETO**. iter 6 compound section (RMSE 2017 = 0.19m). iter 7 validation temporal 1998 (RMSE 1.30m, compound 2-stage satura al peak Q — cross-event gap detectado). iter 8 **power-law cross-section + multi-param AD joint calibration**: RMSE 2017 = 0.006m, RMSE 1998 = 0.103m (12.6× mejor que iter 7). Cross-event gap CERRADO. Differentiable cross-section parameterization es la solución metodológica. Trade-off honesto: n recuperado fuera del Chow envelope por n-shape confound, requiere geometría independiente para desambiguar (limitación de DATA, no de método). ~238 tests verde + 9 demos AD ejecutables.
Próximo milestone: Track A COMPLETO — paper draft del 2028 Q1 con resultados existentes, o Track C alternativo (lavapipe/cloud), o pulir n-shape confound con geometría independiente cuando llegue.
...
> *Versión canónica del wedge — citable directamente en README, intro de papers, propuestas Fondecyt. Cambios sustantivos a este párrafo deben propagarse en paralelo a `README.md` y `state-of-the-art.md`. La versión "intersección residual con 3 entrantes" (2026-05-19) fue retractada el 2026-06-04 cuando una pasada de verify-refs vía CrossRef/OpenAlex confirmó que dos de los tres entrantes citados — Hydrograd.jl y AegirJAX — no existen como publicaciones reales. Solo SynxFlow es verificable.*

**hydroflux es un solver 2D shallow-water diferenciable por construcción en un lenguaje compilado, verificado contra la jerarquía estándar y aplicado a una cuenca andina semiárida.** La diferenciabilidad ha emergido como framework unificador en geociencias [Shen 2023] y la hidrología la ha adoptado tempranamente [Feng et al. 2022; Tsai et al. 2021]. En el núcleo de fluidos, JAX-Fluids [Bezgin et al. 2023] entrega un solver CFD de alto orden completamente diferenciable en JAX; del lado multi-peligro, SynxFlow [Xia et al. 2025] acopla flood-landslide-debris en un engine GPU. El paradigma compartido: el *forward solver* se vuelve diferenciable (o GPU-nativo, o ambos), y los problemas inversos heredan gradientes eficientes. La capa diferenciable típicamente vive en un host language con AD maduro (JAX, PyTorch, Julia). **hydroflux ocupa el complemento**: un solver 2D shallow-water diferenciable por construcción *en un lenguaje compilado de sistemas*, verificado sobre el suite canónico (Stoker, MacDonald, Thacker, dam-break radial, UK EA ×6) y aplicado a un caso real data-sparse (Aluvión Atacama 2017 sobre Río Huasco). Cuatro decisiones de diseño la marcan: **(i) diferenciabilidad por genericidad numérica** vía un trait `Real` que hace que el mismo código compile a `f64` y a `Dual<f64>` (overhead medido 1.98× sobre f64); **(ii) I/O GIS-nativo** a través del crate hermano SurtGIS, con DEM y land-cover GeoTIFFs entrando directo al solver; **(iii) target GPU multiplataforma vía `wgpu`** (Vulkan/Metal/DX12/WebGPU) — el roadmap explícito tras el descubrimiento honesto de que paralelismo CPU vía rayon es ineficaz a la granularidad per-face de este esquema; **(iv) anclaje en cuencas BNA chilenas** como dominio canónico de aplicación. Las cuatro propiedades son *aditivas* — ninguna es novedad por sí sola; lo distintivo es su composición en un único artefacto open-source desde el inicio.
...
| **2027 Q4** ✅ (scaffolding + application iter 1-8 → COMPLETO) | **TRACK A (Autograd)**: Forward-mode dual numbers ✅. **Application iter 1-8 ✅**: synth → DEM bed → tiempo real → rating curve → DEM width → compound section → validation 1998 (gap detectado) → **power-law section + multi-param AD** (RMSE 2017=0.006m, 1998=0.103m, cross-event gap CERRADO). **Adelantado a 2026-05-24**. Item (d') rating oficial DGA pendiente acceso externo pero no bloquea. |
...
- [x] **Review vivo del state of the art** en `state-of-the-art.md`. 12 fichas iniciales. Una primera versión (2026-05-19) agregó tres "entrantes 2024-2025" tras un literature scout informal; un verify-refs vía CrossRef/OpenAlex el 2026-06-04 confirmó que solo SynxFlow (Xia et al. 2025, JOSS) es publicación real — Hydrograd.jl y AegirJAX no aparecen en ninguna base bibliográfica. Las fichas y el wedge fueron reescritos en consecuencia.
...
- **vs paradigmas vigentes**: la calibración por gradiente en SWE 2D suele estar atada a runtimes managed (JAX/Python, Julia JIT, PyTorch); hydroflux la entrega como binario Rust nativo, clave para uso operacional en DGA/SERNAGEOMIN/MOP.
...
- **Riesgo**: medio — autograd forward-mode ya entregado a junio 2026 (overhead 1.98×); reverse-mode no trivial pero camino bien pavimentado por la literatura de differentiable hydrology.
...
- **vs paradigmas vigentes**: SynxFlow (Xia et al. 2025) tiene GPU pero CUDA-only + GPL. TUFLOW HPC tiene GPU pero comercial + cerrado. LISFLOOD-FP GPU usa inertial approximation. Nadie entrega `wgpu` cross-platform + opensource + continental Chile.
...
| 2026-05-18 | **Pivot estratégico**: literature scout informal de differentiable / GPU SWE solvers 2024-2025 motivó pausar el paper de review Q4 2026 (archive en `papers/01_review/`) y pivotear a desarrollo del artefacto. Primer paper se mueve a 2028 Q1 como methods paper con artifact 2D + autograd, target WRR / GMD / EMS. Rationale: artifact-backed claim es defendible, roadmap-promise no. DICYT obligation cubierta por Paper 2 (U-Net SAR en R2 RSE). Fondecyt 2028 más fuerte con artifact que con review. *Nota 2026-06-04*: el scout 2026-05-18 incluyó dos entrantes ("Hydrograd.jl Liu WRR 2025" y "AegirJAX") que un verify-refs vía CrossRef/OpenAlex confirmó posteriormente como inexistentes; solo SynxFlow (Xia et al. 2025, JOSS) es verificable. La decisión del pivot sigue siendo sólida — el JAX-Fluids real (Bezgin et al. 2023) y la literatura de differentiable hydrology (Shen 2023, Feng 2022, Tsai 2021) ya hacían el claim de novelty pura ("no production-grade flood solver in mature autograd language") difícil de sostener; el artefacto entregado es lo que vuelve defendible el wedge. |
| 2026-05-19 | **Cierre primera iteración solver-2d**: 6 commits (`03e57df → 2a92c6d`), ~2160 LOC, 60 unit tests + 6 integration tests, todos verdes. Building blocks completos (state + flux + geometry + riemann HLLC + boundary 4×4 + update Audusse 2D + source Manning 2D). Benchmark analítico **Thacker 1981** pasa: L² rel error 1.62%, L∞ 2.49% h₀, conservación de masa 8.83e-16 (machine precision). Resultados consistentes con literatura (Liang & Marche 2009: 1%, Brufau et al. 2002: 3%). **Wedge revisado a la luz del pivot 2026-05-18**: la versión 2026-05-16 (intersección amplia open+modern+GPU+diff+coupled) se archiva; el wedge canónico nuevo articula la composición defendible — (i) differentiability by numeric genericity (Real trait), (ii) GIS-native I/O, (iii) GPU-targeted via wgpu, (iv) anclaje en cuencas BNA chilenas. Propagado a `state-of-the-art.md` y `README.md`. *Versión 2026-05-19 del wedge fue posteriormente reescrita el 2026-06-04 al confirmarse via verify-refs que dos de los tres entrantes citados en el "gap final" eran alucinaciones.* |
...
| 2026-05-24 | **Track A iter 8 — differentiable cross-section parameterization** (commit e73300e). Resuelve el cross-event gap de iter 7. Nueva pieza: `Real::powt` (exponente diferenciable, no solo base) + `autograd::power_law_swe1d` con `PowerLawSection<T> {coefficient, exponent}` ambos type-T (calibratable). Cross-section continua `T(h) = c · h^p` (Leopold hydraulic geometry) reemplaza el 2-stage compound. Closed-form A(h), I₁(h), stage(A); wide-channel limit (p=0) recupera swe1d. 7 tests. Demo `calibrate_powerlaw_huasco`: joint calibration de (n, c, p) via forward-mode AD (3 forwards por iter, una con cada param como Dual::variable). 20 iter, 32 min wall time, converged en iter 8 a n=0.0131, c=20.09, p=0.7706. **Resultados**: RMSE 2017 = 0.006 m (32× mejor que iter 6 = 0.190), RMSE 1998 = 0.103 m (12.6× mejor que iter 7 = 1.297). **Hipótesis CONFIRMADA**: cross-section continua que da h ∝ Q^(1/(p+5/3)) en todo el rango de stage sí generaliza cross-event, mientras compound 2-stage saturaba al peak. Trade-off honesto: n=0.0131 cae FUERA del Chow envelope [0.025, 0.080] — power-law absorbió parte de la fricción en el shape (n-shape confound). Resolver el confound requiere geometría independiente (sub-30m DEM o field survey), limitación de DATA no del método. Track A application essentialmente COMPLETO. |
