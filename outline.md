...
Estado: Año 1, solver-2d orden 2 + UK EA suite 6/6 + **Track A scaffolding cerrado (milestone 2027 Q4 ADELANTADO)**. ~213 tests verde. Solver-2d features: MUSCL + SSP-RK2 + Liang & Marche 2009 bed-recon + flux-rescaling + Manning + point source + rain-on-grid. Autograd features: forward-mode `Dual` + `Real` trait + primitivas SWE genéricas + 1D Lax-Friedrichs sobre `T: Real` + demo de calibración Manning (4 iter a precisión de máquina). 4 analytical benchmarks + 6 UK EA-style + AD-vs-FD locking test.
