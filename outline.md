...
Estado: Año 1, solver-2d orden 2 + UK EA suite 6/6 + Track A scaffolding cerrado + lake-at-rest bumpy + Thacker (Castro DESCARGADA) + Discharge-on-dry (UK EA deuda DESCARGADA) + **Track A application iter 1 (synthetic bed) + iter 2 (DEM-derived bed)**: Manning calibration sobre evento Atacama 2017 con forzamiento REAL del DGA, recuperando n=0.04 a 0.04% / 0.1% rel error. ~221 tests verde + 3 demos AD ejecutables. Solver-2d features: MUSCL + SSP-RK2 + LM-2009 bed-recon + flux-rescaling + Manning + point source + rain-on-grid + h_n ghost. Autograd features: `Dual` + `Real` trait + primitivas SWE genéricas + 1D Lax-Friedrichs sobre `T: Real` + Manning calibration demos (sintético precisión máquina + Atacama 2017 synthetic-bed + Atacama 2017 DEM-bed).
Próximo milestone: cierre Track A application — tiempo no comprimido (21 días reales) + validation split aguas arriba + rating curve DGA real (target observado vs twin) + channel width DEM-derived.
...
| **2027 Q4** ✅ (scaffolding + application iter 1 + iter 2 DEM) | **TRACK A (Autograd)**: Forward-mode dual numbers ✅ (crate `autograd` con `Dual`, `Real`, primitivas SWE genéricas, solver SWE 1D LF, demo calibración Manning). **Application iter 1 ✅**: Manning calibration sobre evento Atacama 2017 con forzamiento DGA real, recuperando n=0.04 a 0.04% rel error (bed sintético). **Application iter 2 ✅**: mismo evento sobre bed DEM-derived (Huasco filled.tif → 60 cells × 30.6 m), recuperando n a 0.1% rel error en 40 iter. **Adelantado a 2026-05-23**. Falta para cierre completo: tiempo real + validation aguas arriba + rating curve real + channel width DEM-derived. Decisión de ángulo del paper 2028 Q1 sigue diferida a 2027 Q4 real. |
