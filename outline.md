...
Estado: Año 1, solver-2d orden 2 + UK EA suite 6/6 + Track A scaffolding cerrado + lake-at-rest bumpy + Thacker (Castro DESCARGADA) + Discharge-on-dry (UK EA deuda DESCARGADA) + **Track A application iter 1 (synth bed) + iter 2 (DEM bed) + iter 3 (DEM + tiempo real) + iter 4 (rating curve target)**: Manning calibration sobre evento Atacama 2017. Iter 3 (twin): convergencia 9 iter a |err|=3.14e-8 sobre 600k pasos CFL. Iter 4 (rating curve externa literature-derived): n_recovered=0.0167 fuera de envelope Chow — diagnostica mismatch modelo-observación, confirma que width DEM-derived es crítico. ~221 tests verde + 5 demos AD ejecutables.
Próximo milestone: cierre Track A application — **channel width DEM-derived (HAND-based / stream-network buffer)** + validation split aguas arriba + rating curve oficial DGA.
...
| **2027 Q4** ✅ (scaffolding + application iter 1-4) | **TRACK A (Autograd)**: Forward-mode dual numbers ✅. **Application iter 1 ✅** synth bed, **iter 2 ✅** DEM bed, **iter 3 ✅** DEM + tiempo real, **iter 4 ✅** rating curve target externo (n recuperado 0.0167 fuera de envelope plausible → detecta mismatch modelo-observación, confirma que width DEM-derived es crítico). **Adelantado a 2026-05-23**. Falta para cierre completo: validation aguas arriba + channel width DEM-derived + rating curve oficial DGA. |
